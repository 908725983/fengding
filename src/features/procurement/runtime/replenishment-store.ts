...
export const useReplenishmentStore = defineStore('procurement-replenishment', () => {
  const runtimeController = getApplicationMockRuntimeController(); const session = runtimeController.procurement
  const scenario = computed<ProcurementScenarioName>(() => session.scenarioName)
  ...
  async function setScenario(next: ProcurementScenarioName): Promise<void> { actor.value = next === 'permission-denied' ? { role: 'finance', actorId: 'finance-demo' } : { role: 'super-admin', actorId: 'admin-demo' }; setCurrentProcurementRole(actor.value.role); runtimeController.reset(next); await load() }
