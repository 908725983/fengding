...
const { categories, tags, selectedCustomer, loading, actor } = storeToRefs(store)
...
function clearForm(): void {
  ...
  else setDraft({ ...createEmptyCustomerDraft(), salespersonId: actor.value.actorId })
}
...
async function save(saveAndNew: boolean): Promise<void> {
  ...
  try {
    ...
    if (saveAndNew) {
      ...
      setDraft({ ...createEmptyCustomerDraft(), salespersonId: actor.value.actorId })
      await nextTick()
...
<template>
  <section class="form-page">
    <header class="form-header">
      <div><RouterLink class="back-link" :to="isEdit ? `/customers/${route.params.customerId}` : '/customers'">← 返回</RouterLink><p class="eyebrow">CUS-001 · 客户档案</p><h1>{{ isEdit ? '编辑客户' : '新增客户' }}</h1><p>先填写客户和下单必需信息，其余资料可在客户详情补充。</p></div>
      <div class="form-actions"><button class="button" type="button" @click="clearForm">清空表单</button><button class="button" type="button" :disabled="saving" @click="save(true)">保存并新增</button><button class="button button--primary" type="button" :disabled="saving" @click="save(false)">{{ saving ? '保存中…' : '保存' }}</button></div>
    </header>
    ...
    <form v-if="!loading" class="customer-form" @submit.prevent="save(false)">
      <main class="form-main">
        ...
        <section class="form-card"><h2>联系信息</h2><div class="form-grid">
          ...
          <label><span>城市 *</span><input v-model="draft.cityCode" placeholder="请输入城市" :aria-invalid="Boolean(fieldError('cityCode'))"><em>{{ fieldError('cityCode') }}</em></label>
          ...
          <label><span>邮箱</span><input v-model="draft.email" type="email" :aria-invalid="Boolean(fieldError('email'))"><em>{{ fieldError('email') }}</em></label>
        </div></section>

        <section class="form-card"><h2>财务信息</h2><div class="form-grid">
          <label><span>业务员</span><select v-model="draft.salespersonId" :aria-invalid="Boolean(fieldError('salespersonId'))"><option value="">暂不分配</option><option value="staff-demo-1">演示业务员甲</option><option value="staff-demo-2">演示业务员乙</option></select><em>{{ fieldError('salespersonId') }}</em></label>
          <label><span>信用额度（元）</span><input :value="draft.creditLimitCents === null ? '' : draft.creditLimitCents / 100" type="number" min="0" @input="draft.creditLimitCents = ($event.target as HTMLInputElement).value === '' ? null : Math.round(Number(($event.target as HTMLInputElement).value) * 100)"><small>仅账期客户需要填写</small></label>
          ...
          <p class="form-hint form-wide">收款方式、退款账户和开票资料在实际发生业务时填写，客户建档不强制录入。</p>
        </div></section>

        <section class="form-card"><h2>补充资料（可选）</h2><div class="form-grid"><label class="form-wide"><span>客户简介</span><textarea v-model="draft.description" maxlength="500" rows="3"></textarea><small>{{ draft.description?.length ?? 0 }}/500</small></label><label class="form-wide"><span>备注</span><textarea v-model="draft.remark" rows="3"></textarea></label></div><p class="scope-note">资质、附件和客户经营设置请在客户详情维护。</p></section>
      </main>
