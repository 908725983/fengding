...
describe('PRD-001 product route access', () => {
  it('keeps denial on the list surface and blocks protected subroutes', () => {
    ...
    expect(guardProductSubroute('/products/authorizations/plans')).toBe('/products?denied=1')
    expect(guardProductSubroute('/products/distribution/plans')).toBe('/products?denied=1')
    setCurrentProductRole('sales-supervisor')
    expect(guardProductSubroute('/products/authorizations/plans')).toBe(true)
    expect(guardProductSubroute('/products/authorizations/plans/new')).toBe('/products/authorizations/plans?denied=1')
    expect(guardProductSubroute('/products/distribution/plans')).toBe(true)
    expect(guardProductSubroute('/products/distribution/plans/new')).toBe('/products/distribution/plans?denied=1')
    expect(guardProductSubroute('/products/order-templates/order-template-1/edit')).toBe('/products/distribution/plans?denied=1')
    setCurrentProductRole('super-admin')
    expect(guardProductSubroute('/products/authorizations/rules/new')).toBe(true)
    expect(guardProductSubroute('/products/order-templates/new')).toBe(true)
  })
})
