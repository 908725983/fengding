...
<template>
  <section class="detail-page">
    ...
    <template v-else>
      ...
      <nav class="tabs"><strong>商品描述</strong><span>商品授权 · PRD-003 规划中</span><span>等级/客户价格 · PRD-002 规划中</span><span>供应商价格 · 规划中</span><span>订货设置/关联商品 · 规划中</span></nav>
      <div class="detail-grid"><main>
        ...
        <section class="card"><h2>辅助与物流</h2><dl class="side-fields"><div><dt>重量</dt><dd>{{ product.weightKg === null ? '—' : `${product.weightKg} kg` }}</dd></div><div><dt>产地</dt><dd>{{ display(product.origin) }}</dd></div><div><dt>保质期</dt><dd>{{ product.shelfLifeDays === null ? '—' : `${product.shelfLifeDays} 天` }}</dd></div><div><dt>起订量</dt><dd>{{ display(product.minimumOrderQuantity) }}</dd></div><div><dt>销售税率</dt><dd>{{ product.salesTaxRatePercent === null ? '—' : `${product.salesTaxRatePercent}%` }}</dd></div><div><dt>包邮</dt><dd>{{ product.freeShipping ? '是' : '否' }}</dd></div><div><dt>运费模板</dt><dd>数据源未接入</dd></div></dl></section>
