...
import type { Product, ProductCategory, ProductDraft, ProductMedia, ProductSkuDraft, RichTextDocument } from '../types'
...
type CategoryRow = { category: ProductCategory; depth: number; hasChildren: boolean }
...
const categoryRows = computed<CategoryRow[]>(() => {
  ...
  const children = (parentId: string | null, depth: number): CategoryRow[] => categories
    ...
    .flatMap((category) => [{ category, depth, hasChildren: categories.some((item) => String(item.parentId ?? '') === String(category.id)) }, ...children(String(category.id), depth + 1)])
...
const selectedCategory = computed(() => references.value.categories.find((item) => String(item.id) === String(draft.categoryId)))
function openCategoryMenu(): void {
  categoryOpen.value = true
  let parentId = selectedCategory.value?.parentId ? String(selectedCategory.value.parentId) : null
  while (parentId) {
    expandedCategoryIds.value.add(parentId)
    const parent = references.value.categories.find((category) => String(category.id) === parentId)
    parentId = parent?.parentId ? String(parent.parentId) : null
  }
}
function toggleCategory(item: CategoryRow): void {
  ...
  if (item.category.status === 'active') { draft.categoryId = String(item.category.id); categoryOpen.value = false }
}
...
<template>
  <section class="form-page"><header class="form-header"><div><RouterLink class="back" :to="isEdit?`/products/${route.params.productId}`:'/products'">← 返回</RouterLink><p class="eyebrow">PRD-001 · 商品档案</p><h1>{{ isEdit?'编辑商品':'新增商品' }}</h1><p>价格按 SKU 保存；最终客户价格、授权和拼套商品不属于本切片。</p></div><div v-if="canWrite" class="actions"><button class="button" type="button" @click="clearForm">清空表单</button><button class="button" type="button" @click="showTutorial">视频教程</button><button class="button" :disabled="saving" type="button" @click="save(true)">保存并新增</button><button class="button primary" :disabled="saving" type="button" @click="save(false)">{{ saving?'保存中…':'保存' }}</button></div></header>
    ...
    <form v-if="!loading&&canWrite" class="product-form" @submit.prevent="save(false)"><main>
      <section class="card"><h2>商品信息</h2><div class="grid"><label><span>商品名称 *</span><input v-model="draft.name" maxlength="80" :aria-invalid="Boolean(fieldError('name'))"><em>{{fieldError('name')}}</em></label><label class="category-field"><span>商品分类 *</span><div class="category-picker"><button class="category-picker-trigger" type="button" :aria-expanded="categoryOpen" :aria-invalid="Boolean(fieldError('categoryId'))" @click="categoryOpen ? categoryOpen = false : openCategoryMenu()">{{ selectedCategory?.name ?? '请选择分类' }}<span aria-hidden="true">{{ categoryOpen ? '▴' : '▾' }}</span></button><div v-if="categoryOpen" class="category-picker-menu" role="listbox"><button v-for="item in visibleCategoryRows" :key="item.category.id" type="button" class="category-option" :class="{ parent: item.hasChildren, inactive: item.category.status !== 'active' }" :style="{ paddingLeft: `${10 + item.depth * 18}px` }" @click="toggleCategory(item)">{{ item.hasChildren ? (expandedCategoryIds.has(String(item.category.id)) ? '▾ ' : '▸ ') : '· ' }}{{item.category.name}}{{item.category.status === 'inactive' ? '（停用）' : ''}}</button><p v-if="!visibleCategoryRows.length" class="category-empty">暂无可选分类</p></div></div><em>{{fieldError('categoryId')}}</em></label><label><span>基本单位 *</span><select v-model="draft.baseUnitId" :aria-invalid="Boolean(fieldError('baseUnitId'))" @change="onBaseUnitChange"><option value="">请选择单位</option><option v-for="item in references.units.filter(i=>i.status==='active')" :key="item.id" :value="item.id">{{item.name}}</option></select><em>{{fieldError('baseUnitId')}}</em></label><label><span>商品品牌</span><select v-model="draft.brandId"><option :value="null">请选择</option><option v-for="item in references.brands.filter(i=>i.status==='active')" :key="item.id" :value="item.id">{{item.name}}</option></select></label></div><p class="scope">点击父分类展开或收起子分类；商品只能选择具体子分类或没有子分类的分类。</p></section>
...
.category-picker{position:relative}.category-picker-trigger{display:flex;width:100%;min-height:36px;align-items:center;justify-content:space-between;padding:6px 9px;color:var(--color-text);text-align:left;background:#fff;border:1px solid var(--color-border-strong);border-radius:5px}.category-picker-trigger[aria-invalid=true]{border-color:var(--color-danger)}.category-picker-menu{position:absolute;z-index:10;top:calc(100% + 4px);right:0;left:0;max-height:250px;overflow:auto;padding:4px 0;background:#fff;border:1px solid var(--color-border-strong);border-radius:5px;box-shadow:0 8px 22px rgb(20 34 46 / 14%)}.category-option{display:block;width:100%;min-height:34px;padding:6px 10px;color:var(--color-text);text-align:left;background:#fff;border:0}.category-option:hover{background:var(--color-primary-soft)}.category-option.parent{font-weight:600}.category-option.inactive{color:#9aa3ad;cursor:not-allowed}.category-empty{padding:8px 10px;margin:0;color:var(--color-muted)}
