...
type CategoryRow = { category: ProductReference; depth: number }
const categoryRows = computed<CategoryRow[]>(() => {
  ...
  const children = (parentId: string | null, depth: number): CategoryRow[] => categories
    ...
    .flatMap((category) => [{ category, depth }, ...children(String(category.id), depth + 1)])
