...
async function advanceClock(): Promise<void> {
  ...
  try { await store.advanceClock(`${clockTarget.value}:00+08:00`) }
  catch (caught) { actionError.value = caught instanceof Error ? caught.message : '模拟时钟推进失败' }
}
...
<template>
  <section class="price-page" :aria-labelledby="`${type}-price-title`">
    <header class="page-header"><div><p class="eyebrow">PRD-002 · 价格体系与调价</p><h1 :id="`${type}-price-title`">{{ labels[type].title }}</h1><p>{{ labels[type].description }}</p></div><div class="header-tools"><label><span>模拟场景</span><select :value="scenario" @change="switchScenario"><option value="normal">正常</option><option value="empty">空数据</option><option value="error">服务错误</option><option value="slow">慢响应</option><option value="permission-denied">无权限</option><option value="partial-failure">部分资料失败</option><option value="boundary">生效分钟边界</option></select></label><RouterLink v-if="canWrite" class="button primary" :to="`/products/prices/${type}-adjustments/new`">新增调价单</RouterLink></div></header>
    <PriceSubnav />
    <div class="clock-bar"><strong>模拟时钟</strong><span>{{ displayTime(clock || '---- -- --T--:--') }}</span><small>调价只由此可控时钟生效，不使用真实后台定时器。</small><input v-if="canWrite" v-model="clockTarget" aria-label="推进模拟时钟到" type="datetime-local" :min="clock.slice(0,16)"><button v-if="canWrite" class="button" :disabled="!clockTarget || store.saving" type="button" @click="advanceClock">推进时钟</button></div>
    ...
    <p v-if="actionError" class="warning error" role="alert">{{ actionError }}</p>
    ...
    <template v-else><div class="table-tools"><span>共 {{ adjustments.total }} 张单据</span><button class="button" type="button" @click="store.reload">刷新</button></div><div class="table-wrap"><table><thead><tr><th>单号</th><th v-if="type==='customer'">客户名称</th><th>生成日期</th><th>状态</th><th>是否生效</th><th>制单人</th><th>制单时间</th><th>生效时间</th><th>备注</th><th>明细</th></tr></thead><tbody><tr v-for="item in adjustments.items" :key="item.id"><td class="mono"><RouterLink :to="`/products/prices/${type}-adjustments/${item.id}`">{{ item.number }}</RouterLink></td><td v-if="type==='customer'">{{ store.customerName(item.customerId) }}</td><td>{{ item.createdAt.slice(0,10) }}</td><td><span class="status" :class="`status-${item.status}`">{{ statusLabels[item.status] }}</span></td><td>{{ item.status==='effective'?'是':'否' }}</td><td>{{ item.createdBy }}</td><td>{{ displayTime(item.createdAt) }}</td><td>{{ displayTime(item.effectiveAt) }}</td><td class="note">{{ item.note || '—' }}</td><td>{{ item.lines.length }} 行</td></tr></tbody></table></div><footer class="pagination"><span>第 {{ adjustments.page }} 页</span><button class="button" :disabled="adjustments.page<=1" type="button" @click="store.setPage(adjustments.page-1)">上一页</button><button class="button" :disabled="adjustments.page*adjustments.pageSize>=adjustments.total" type="button" @click="store.setPage(adjustments.page+1)">下一页</button></footer></template>
  </section>
</template>

<style scoped>
.price-page{max-width:1680px;margin:0 auto}.page-header{display:flex;justify-content:space-between;gap:20px;margin-bottom:14px}.page-header h1{margin:3px 0}.page-header p{margin-bottom:0;color:var(--color-muted)}.header-tools{display:flex;align-items:flex-end;gap:10px}.header-tools label,.filters label{display:grid;gap:4px;color:var(--color-muted);font-size:12px}select,input{min-height:36px;padding:6px 9px;background:#fff;border:1px solid var(--color-border-strong);border-radius:var(--radius-sm)}.button{display:inline-flex;align-items:center;min-height:36px;padding:0 13px;color:#4d5968;background:#fff;border:1px solid var(--color-border-strong);border-radius:var(--radius-sm);cursor:pointer;text-decoration:none}.button.primary{color:#fff;background:var(--color-primary);border-color:var(--color-primary)}.button:disabled{opacity:.5;cursor:not-allowed}.clock-bar{display:flex;align-items:center;gap:12px;padding:10px 15px;color:#53616f;background:#f4faf9;border:1px solid #cfe6e3;border-top:0}.clock-bar>span{font-family:"Cascadia Code",Consolas,monospace}.clock-bar small{margin-left:auto;color:var(--color-muted)}.clock-bar input{min-height:32px}.filters{display:grid;grid-template-columns:repeat(4,minmax(150px,1fr));gap:12px;padding:14px 16px;background:#fff;border:1px solid var(--color-border);border-top:0}.filter-actions{display:flex;align-items:flex-end;justify-content:flex-end;gap:8px}.warning{padding:9px 12px;color:#8a640d;background:#fff8e8;border:1px solid #eed99b}.state{display:grid;min-height:270px;place-items:center;align-content:center;gap:8px;margin-top:12px;background:#fff;border:1px solid var(--color-border)}.state p{margin:0}.state.error{color:var(--color-danger)}.spinner{width:24px;height:24px;border:3px solid #cfe8e6;border-top-color:var(--color-primary);border-radius:50%;animation:spin .8s linear infinite}@keyframes spin{to{transform:rotate(360deg)}}.table-tools{display:flex;align-items:center;justify-content:space-between;margin-top:12px;padding:10px 12px;background:#fff;border:1px solid var(--color-border);border-bottom:0}.table-tools span{color:var(--color-muted)}.table-wrap{overflow-x:auto;background:#fff;border:1px solid var(--color-border)}table{width:100%;min-width:1250px;border-collapse:collapse}th,td{height:50px;padding:8px 11px;text-align:left;white-space:nowrap;border-bottom:1px solid var(--color-border)}th{color:#4d5968;font-size:12px;background:var(--color-table-head)}td a{color:var(--color-primary-strong);text-decoration:none}.mono{font-family:"Cascadia Code",Consolas,monospace}.note{max-width:260px;overflow:hidden;text-overflow:ellipsis}.status{display:inline-flex;padding:3px 8px;border-radius:999px;font-size:12px}.status-pending{color:#8a640d;background:#fff7e6}.status-effective{color:var(--color-success);background:var(--color-success-soft)}.status-expired{color:#697586;background:#eef1f5}.pagination{display:flex;justify-content:flex-end;align-items:center;gap:8px;padding:11px;background:#fff;border:1px solid var(--color-border);border-top:0}.pagination span{margin-right:8px;color:var(--color-muted)}@media(max-width:1050px){.filters{grid-template-columns:repeat(2,minmax(0,1fr))}.page-header{display:block}.header-tools{margin-top:12px}.clock-bar small{margin-left:0}}@media(max-width:650px){.filters{grid-template-columns:1fr}.header-tools{align-items:stretch;flex-direction:column}.clock-bar{align-items:flex-start;flex-direction:column}}
</style>
