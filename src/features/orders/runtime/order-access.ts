...
export function guardOrderSubroute(path:string):true|string{if(path.startsWith('/share/orders/'))return true;if(!path.startsWith('/orders'))return true;if(!canCurrentRoleAccessOrders())return '/dashboard?denied=orders';if((path==='/orders/new'||/^\/orders\/[^/]+\/edit$/.test(path))&&!canCurrentRoleManageOrders())return '/orders?denied=manage';return true}
