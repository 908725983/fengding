...
export function guardOrderSubroute(path:string):true|string{if(path.startsWith('/share/orders/'))return true;if(!path.startsWith('/orders'))return true;if(!canCurrentRoleAccessOrders())return '/dashboard?denied=orders';if(path==='/orders/statistics'&&currentRole==='warehouse')return '/orders/sales-statistics?denied=statistics-amounts';if(path.startsWith('/orders/differences')&&!['super-admin','sales-supervisor'].includes(currentRole))return '/orders?denied=difference';if(path==='/orders/returns/new'&&!['super-admin','salesperson'].includes(currentRole))return '/orders/returns?denied=create-return';if(/^\/orders\/returns\/[^/]+\/edit$/.test(path)&&!['super-admin','sales-supervisor'].includes(currentRole))return '/orders/returns?denied=edit-return';if((path==='/orders/new'||/^\/orders\/(?!returns\/)[^/]+\/edit$/.test(path))&&!canCurrentRoleManageOrders())return '/orders?denied=manage';return true}
