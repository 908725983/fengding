...
async function reload() { await store.loadReturnDetail(String(route.params.returnId)); if (!store.returnLocations.some((item) => item.id === locationId.value)) locationId.value = store.returnLocations[0]?.id ?? '' }
...
<template>
  <section class="order-page">
    ...
    <template v-else>
      ...
      <section v-if="value.status==='approved' && value.receivingStatus==='pending' && ['super-admin','warehouse'].includes(store.actor.role)" class="order-card"><h2>确认退货入库</h2><label class="order-review-reason">实际库位<select v-model="locationId"><option v-for="item in store.returnLocations" :key="item.id" :value="item.id">{{ item.name }}</option></select></label><small>仅显示退货仓库“{{ value.warehouseSnapshot.name }}”下已启用的库位；新建库位后刷新本页即可同步。</small><button class="order-button primary" :disabled="store.saving || !locationId" @click="inbound">确认入库并形成应收贷项</button></section>
