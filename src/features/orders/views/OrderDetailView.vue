...
async function resolveDifference(id:string,outcome:'reship'|'ignore'|'refund'){if(await store.resolveDifference(id,outcome))message.value=outcome==='reship'?'差异已转补发':outcome==='refund'?'短装退款已确认，发货时形成贷项':'已接受短装，可继续发货'}
  ...
  <section class="order-page">
    ...
    <template v-else
      ...
      <main v-else-if="tab==='fulfillment'&&fulfillmentDetail" class="order-detail-body">
        ...
        <section v-if="fulfillmentDetail.differences.length" class="order-card"><h2>差异单</h2><div class="order-table-wrap"><table class="order-table"><thead><tr><th>差异单</th><th>原因</th><th class="order-money">差异金额</th><th>状态 / 处理</th></tr></thead><tbody><tr v-for="item in fulfillmentDetail.differences" :key="item.id"><td><RouterLink :to="`/orders/differences/${item.id}`">{{ item.differenceNo }}</RouterLink></td><td>{{ item.reason }}</td><td class="order-money">{{ fulfillmentMoney(item.differenceAmountCents) }}</td><td>{{ item.status==='pending-confirmation'?'待确认':item.outcome==='reship'?'已转补发':item.outcome==='ignore'?'已忽略':item.outcome==='refund'?'短装退款':'已作废' }}<div v-if="item.status==='pending-confirmation'&&hasPermission('orders.confirm-difference')" class="order-detail-actions"><button class="order-row-link" @click="resolveDifference(item.id,'reship')">补发</button><button class="order-row-link" @click="resolveDifference(item.id,'ignore')">忽略</button><button class="order-row-link" @click="resolveDifference(item.id,'refund')">短装退款</button></div></td></tr></tbody></table></div></section>
