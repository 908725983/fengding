...
const reviewActions = new Set(['approve-order','return-order','approve-finance','return-finance','cancel-order'])
const reviewStages = new Set(['order','finance'])
const reviewOutcomes = new Set(['approved','returned','canceled'])
const specialDirections = new Set(['below-minimum','above-maximum'])
...
export function validateOrderDraft(draft: OrderDraft): OrderValidationIssue[] {
  ...
  if(draft.specialPrice)required(issues,'specialPriceReason',draft.specialPriceReason??'',200)
  else if(draft.specialPriceReason?.trim())issues.push({path:'specialPriceReason',message:'非特价订单不能填写特价原因'})
...
export function validateOrder(order: CustomerOrder): OrderValidationIssue[] {
  ...
  if(order.specialPrice){required(issues,'specialPriceReason',order.specialPriceReason??'',200)}else{if(order.specialPriceReason?.trim())issues.push({path:'specialPriceReason',message:'非特价订单不能保留特价原因'});if(order.specialPriceEvidence?.length)issues.push({path:'specialPriceEvidence',message:'非特价订单不能保留特价依据'})}
  if(order.reviewRound!==undefined&&(!Number.isSafeInteger(order.reviewRound)||order.reviewRound<1))issues.push({path:'reviewRound',message:'必须是正整数'})
  ...
  ;(order.specialPriceEvidence??[]).forEach((item,index)=>{const base=`specialPriceEvidence.${index}`;if(!lineIds.has(item.lineId))issues.push({path:`${base}.lineId`,message:'关联商品行不存在'});required(issues,`${base}.skuId`,item.skuId,100);required(issues,`${base}.skuCodeSnapshot`,item.skuCodeSnapshot,80);required(issues,`${base}.unitId`,item.unitId,100);required(issues,`${base}.unitNameSnapshot`,item.unitNameSnapshot,80);nonnegative(issues,`${base}.dealUnitPriceCents`,item.dealUnitPriceCents);if(item.minimumSalePriceCents!==null)nonnegative(issues,`${base}.minimumSalePriceCents`,item.minimumSalePriceCents);if(item.maximumSalePriceCents!==null)nonnegative(issues,`${base}.maximumSalePriceCents`,item.maximumSalePriceCents);if(!specialDirections.has(item.direction))issues.push({path:`${base}.direction`,message:'越界方向无效'});if(item.direction==='below-minimum'&&(item.minimumSalePriceCents===null||item.dealUnitPriceCents>=item.minimumSalePriceCents))issues.push({path:base,message:'低于最低价依据不成立'});if(item.direction==='above-maximum'&&(item.maximumSalePriceCents===null||item.dealUnitPriceCents<=item.maximumSalePriceCents))issues.push({path:base,message:'高于最高价依据不成立'})})
  const reviewIds=new Set<string>();(order.reviewRecords??[]).forEach((item,index)=>{const base=`reviewRecords.${index}`;if(reviewIds.has(item.id))issues.push({path:`${base}.id`,message:'审核记录ID重复'});reviewIds.add(item.id);required(issues,`${base}.id`,item.id,100);if(!Number.isSafeInteger(item.round)||item.round<1)issues.push({path:`${base}.round`,message:'轮次必须为正整数'});if(item.round>(order.reviewRound??1))issues.push({path:`${base}.round`,message:'不能大于订单当前审核轮次'});if(!reviewStages.has(item.stage))issues.push({path:`${base}.stage`,message:'审核阶段无效'});if(!reviewOutcomes.has(item.outcome))issues.push({path:`${base}.outcome`,message:'审核结果无效'});required(issues,`${base}.actorSnapshot.id`,item.actorSnapshot.id,100);required(issues,`${base}.actorSnapshot.name`,item.actorSnapshot.name,100);if(!statuses.has(item.fromStatus)||!statuses.has(item.toStatus))issues.push({path:base,message:'审核状态无效'});if(item.reason!==null&&item.reason.trim().length>200)issues.push({path:`${base}.reason`,message:'不能超过200个字符'});if((item.outcome==='returned'||item.outcome==='canceled')&&!item.reason?.trim())issues.push({path:`${base}.reason`,message:'退回或取消必须填写原因'});nonnegative(issues,`${base}.releasedPrepaymentCents`,item.releasedPrepaymentCents);if(!iso(item.occurredAt))issues.push({path:`${base}.occurredAt`,message:'必须是ISO时间'});required(issues,`${base}.requestId`,item.requestId,100)})
...
export function assertOrderFeatureState(state: OrderFeatureState): void {
  ...
  const reviewRequestIds=(state.reviewRequests??[]).map((item)=>item.requestId);if(new Set(reviewRequestIds).size!==reviewRequestIds.length)issues.push({path:'reviewRequests.requestId',message:'必须唯一'});(state.reviewRequests??[]).forEach((item,index)=>{required(issues,`reviewRequests.${index}.requestId`,item.requestId,100);if(!reviewActions.has(item.action))issues.push({path:`reviewRequests.${index}.action`,message:'动作无效'});if(!item.orderIds.length||new Set(item.orderIds).size!==item.orderIds.length)issues.push({path:`reviewRequests.${index}.orderIds`,message:'订单必须非空且唯一'});item.orderIds.forEach((id)=>{if(!ids.includes(id))issues.push({path:`reviewRequests.${index}.orderIds`,message:'订单不存在'})});if(!iso(item.appliedAt))issues.push({path:`reviewRequests.${index}.appliedAt`,message:'必须是ISO时间'})})
