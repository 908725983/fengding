...
import type {
  ...
  SupplierPayable, SupplierPayableProjection, SupplierPayableProductRow, SupplierPayableAgingRow, SupplierPayment,
  SupplierPaymentWriteoff, SupplierPayableCredit, CreateSupplierPayableInput, CreateSupplierPaymentInput,
  VoidSupplierPaymentInput, CreateSupplierPaymentWriteoffInput, CancelSupplierPaymentWriteoffInput, CreateSupplierPayableCreditInput,
} from '../types'
...
export interface FinanceServiceDependencies {
  ...
  now: () => string
  nextId: (kind: 'account' | 'movement' | 'period' | 'bank' | 'application' | 'audit' | 'receivable' | 'receipt' | 'writeoff' | 'allocation' | 'prepayment' | 'payable' | 'payment' | 'supplier-writeoff' | 'supplier-credit') => string
...
const rolePermissions: Record<FinanceActor['role'], FinancePermission[]> = {
  ...
  'sales-supervisor': ['finance.view-accounts', 'finance.view-ledger', 'finance.view-receivables', 'finance.view-receivable-details', 'finance.view-aging', 'finance.view-receipts', 'finance.view-writeoffs', 'finance.export-receivables', 'finance.view-payables', 'finance.view-payable-details', 'finance.view-payable-aging', 'finance.view-supplier-payments', 'finance.view-supplier-writeoffs', 'finance.export-payables'], salesperson: [], warehouse: [],
}
...
function nextBusinessNo(state: FinanceFeatureState, kind: 'receivable' | 'receipt' | 'writeoff' | 'payable' | 'payment', at: string): string {
  const date = datePart(at); let sequence = state.dailySequences.find((item) => item.date === date)
  if (!sequence) { sequence = { date, receivable: 1, receipt: 1, writeoff: 1, payable: 1, payment: 1 }; state.dailySequences.push(sequence) }
  const current = sequence[kind] ?? 1; sequence[kind] = current + 1
  const prefix = kind === 'receivable' ? 'YS' : kind === 'receipt' ? 'SK' : kind === 'writeoff' ? 'HX' : kind === 'payable' ? 'YF' : 'FK'
...
function customerSnapshot(value: CreateCustomerReceiptInput['customerSnapshot']): CreateCustomerReceiptInput['customerSnapshot'] { return { id: value.id, code: value.code, name: value.name, contactName: value.contactName, phone: value.phone } }
function supplierSnapshot(value: CreateSupplierPayableInput['supplierSnapshot']): CreateSupplierPayableInput['supplierSnapshot'] { return { id: value.id, code: value.code, name: value.name, contactName: value.contactName, phone: value.phone, paymentTermDays: value.paymentTermDays } }
function supplierPaidCents(state: FinanceFeatureState, payableId: string): number { return state.supplierPaymentWriteoffs.filter((item) => item.status === 'active').flatMap((item) => item.allocations).filter((item) => item.payableId === payableId).reduce((sum, item) => sum + item.amountCents, 0) }
function supplierCreditedCents(state: FinanceFeatureState, payableId: string): number { return state.supplierPayableCredits.filter((item) => item.status === 'active' && item.payableId === payableId).reduce((sum, item) => sum + item.outstandingReductionCents, 0) }
function projectSupplierPayable(state: FinanceFeatureState, value: SupplierPayable, now: string): SupplierPayableProjection {
  const paidCents = supplierPaidCents(state, value.id); const creditedCents = supplierCreditedCents(state, value.id); const outstandingCents = Math.max(0, value.amountCents - paidCents - creditedCents)
  const status = value.status === 'voided' ? 'voided' : outstandingCents === 0 ? 'paid' : paidCents + creditedCents > 0 ? 'partially-paid' : 'unpaid'
  return { ...structuredClone(value), paidCents, creditedCents, outstandingCents, status, ageDays: ageDays(value.occurredAt, now), overdue: outstandingCents > 0 && datePart(now) > value.dueDate }
}
function supplierPaymentAvailable(state: FinanceFeatureState, paymentId: string): number { const payment = state.supplierPayments.find((item) => item.id === paymentId); if (!payment || payment.status === 'void') return 0; const allocated = state.supplierPaymentWriteoffs.filter((item) => item.status === 'active').flatMap((item) => item.allocations).filter((item) => item.paymentId === paymentId).reduce((sum, item) => sum + item.amountCents, 0); return Math.max(0, payment.amountCents - allocated) }
...
export function createFinanceService(deps: FinanceServiceDependencies) {
  ...
  function listSettlementSources(actor: FinanceActor, customerId: string): SettlementSource[] {
    permission(actor, 'finance.view-writeoffs'); const state = deps.repository.read(); const positiveBySource = new Map<string, CustomerPrepaymentLedgerEntry>(); for (const item of state.prepaymentLedger.filter((entry) => entry.customerId === customerId && entry.amountDeltaCents > 0)) if (!positiveBySource.has(item.sourceId)) positiveBySource.set(item.sourceId, item); const positives = [...positiveBySource.values()]
    return positives.map((entry): SettlementSource => { const receipt = state.customerReceipts.find((item) => item.id === entry.sourceId); return { kind: receipt ? 'receipt' : 'prepayment', id: entry.sourceId, label: receipt?.receiptNo ?? (entry.bucket === 'occupied' ? `订单占用预收 ${entry.orderId}` : '可用预收'), customerId, availableCents: sourceBalance(state, entry.sourceId), occurredAt: entry.occurredAt, orderId: entry.orderId, bucket: entry.bucket } }).filter((item) => item.availableCents > 0).sort((a, b) => a.occurredAt.localeCompare(b.occurredAt) || a.id.localeCompare(b.id))
  }
  ...
  function createReceipt(actor: FinanceActor, input: CreateCustomerReceiptInput): { receipt: CustomerReceipt; writeoff: ReceiptWriteoff | null } {
    ...
    return deps.repository.transact((state) => {
      ...
      const receipt: CustomerReceipt = { id: receiptId, enterpriseId: state.enterpriseId, receiptNo, customerSnapshot: customerSnapshot(input.customerSnapshot), orderId: input.orderId ?? null, occurredAt: input.occurredAt, amountCents: input.amountCents, method: input.method, accountId: account?.id ?? null, movementId: movement?.id ?? null, attachment: input.attachment ? { id: input.attachment.id, name: input.attachment.name, mimeType: input.attachment.mimeType, sizeBytes: input.attachment.sizeBytes } : null, note: input.note?.trim() || null, status: 'normal', voidInfo: null, requestId: input.requestId, operatorSnapshot: actorSnapshot(actor, deps), version: 1 }
  ...
  function createSupplierPayable(actor: FinanceActor, input: CreateSupplierPayableInput): SupplierPayable {
    permission(actor, 'finance.manage-supplier-payments'); assertOccurredAt(input.occurredAt, deps.now(), 'occurredAt')
    if (!input.requestId.trim() || !input.purchaseOrderId.trim() || !input.inboundId.trim() || !input.purchaseOrderNo.trim() || !input.inboundNo.trim()) throw new FinanceValidationError([{ path: 'source', message: '采购入库来源不能为空' }])
    if (!input.items.length) throw new FinanceValidationError([{ path: 'items', message: '至少需要一条入库明细' }])
    if (![input.goodsAmountCents, input.discountCents, input.otherFeeCents, input.amountCents].every((value) => Number.isSafeInteger(value) && value >= 0) || input.amountCents !== input.goodsAmountCents - input.discountCents + input.otherFeeCents) throw new FinanceValidationError([{ path: 'amountCents', message: '应付金额必须为非负整数分且满足商品金额-优惠+其他费用' }])
    return deps.repository.transact((state) => {
      const prior = replay(state, input.requestId, 'payable-create', () => state.payables.find((item) => item.id === state.requests.find((entry) => entry.requestId === input.requestId)!.targetIds[0])!); if (prior) return prior
      if (state.payables.some((item) => item.inboundId === input.inboundId)) throw new FinanceDomainError('DUPLICATE', '该入库已经形成应付')
      const dueDate = input.paymentTermDays && input.paymentTermDays > 0 ? addDays(datePart(input.occurredAt), input.paymentTermDays) : datePart(input.occurredAt)
      const value: SupplierPayable = { id: deps.nextId('payable'), enterpriseId: state.enterpriseId, payableNo: nextBusinessNo(state, 'payable', input.occurredAt), source: 'purchase-inbound', purchaseOrderId: input.purchaseOrderId, purchaseOrderNo: input.purchaseOrderNo.trim(), inboundId: input.inboundId, inboundNo: input.inboundNo.trim(), supplierSnapshot: supplierSnapshot(input.supplierSnapshot), items: structuredClone(input.items), goodsAmountCents: input.goodsAmountCents, discountCents: input.discountCents, otherFeeCents: input.otherFeeCents, amountCents: input.amountCents, occurredAt: input.occurredAt, dueDate, status: input.amountCents === 0 ? 'paid' : 'unpaid', requestId: input.requestId, createdBy: actorSnapshot(actor, deps), version: 1 }
      state.payables.push(value); state.version += 1; recordRequest(state, input.requestId, 'payable-create', [value.id], deps.now()); state.auditLogs.push({ id: deps.nextId('audit'), enterpriseId: state.enterpriseId, action: 'payable.created', targetId: value.id, operatorSnapshot: actorSnapshot(actor, deps), detail: `入库 ${value.inboundNo} 形成应付 ${value.payableNo}`, createdAt: deps.now() }); return value
    })
  }

  function listSupplierPayables(actor: FinanceActor, query: { keyword?: string; startDate?: string; endDate?: string } = {}): SupplierPayableProjection[] {
    permission(actor, 'finance.view-payable-details'); const keyword = normalize(query.keyword ?? ''); const state = deps.repository.read()
    return state.payables.filter((item) => (!keyword || [item.payableNo, item.purchaseOrderNo, item.inboundNo, item.supplierSnapshot.code, item.supplierSnapshot.name].some((value) => normalize(value).includes(keyword))) && (!query.startDate || datePart(item.occurredAt) >= query.startDate) && (!query.endDate || datePart(item.occurredAt) <= query.endDate)).sort((a, b) => b.occurredAt.localeCompare(a.occurredAt) || b.id.localeCompare(a.id)).map((item) => { const value = projectSupplierPayable(state, item, deps.now()); if (actor.role === 'sales-supervisor') value.supplierSnapshot.phone = maskPhone(value.supplierSnapshot.phone); return value })
  }
  function listSupplierPayableProducts(actor: FinanceActor, query: { keyword?: string } = {}): SupplierPayableProductRow[] {
    return listSupplierPayables(actor, query).flatMap((payable) => payable.items.filter((item) => !query.keyword || normalize(`${item.skuCode} ${item.productName}`).includes(normalize(query.keyword!))).map((item) => ({ ...structuredClone(item), payableId: payable.id, payableNo: payable.payableNo, purchaseOrderNo: payable.purchaseOrderNo, occurredAt: payable.occurredAt, supplier: structuredClone(payable.supplierSnapshot), status: payable.status })))
  }
  function listSupplierPayableAging(actor: FinanceActor): SupplierPayableAgingRow[] {
    permission(actor, 'finance.view-payable-aging'); const groups = new Map<string, SupplierPayableProjection[]>(); listSupplierPayables(actor).filter((item) => item.outstandingCents > 0).forEach((item) => groups.set(item.supplierSnapshot.id, [...(groups.get(item.supplierSnapshot.id) ?? []), item]))
    return [...groups.values()].map((items) => { const row: SupplierPayableAgingRow = { supplier: structuredClone(items[0]!.supplierSnapshot), outstandingCents: 0, bucket0To30Cents: 0, bucket31To60Cents: 0, bucket61To90Cents: 0, bucket91To180Cents: 0, bucket181To365Cents: 0, bucketOver365Cents: 0 }; for (const item of items) { row.outstandingCents += item.outstandingCents; if (item.ageDays <= 30) row.bucket0To30Cents += item.outstandingCents; else if (item.ageDays <= 60) row.bucket31To60Cents += item.outstandingCents; else if (item.ageDays <= 90) row.bucket61To90Cents += item.outstandingCents; else if (item.ageDays <= 180) row.bucket91To180Cents += item.outstandingCents; else if (item.ageDays <= 365) row.bucket181To365Cents += item.outstandingCents; else row.bucketOver365Cents += item.outstandingCents } return row }).sort((a, b) => b.outstandingCents - a.outstandingCents)
  }

  function createSupplierPayment(actor: FinanceActor, input: CreateSupplierPaymentInput): { payment: SupplierPayment; writeoff: SupplierPaymentWriteoff | null } {
    permission(actor, 'finance.manage-supplier-payments'); assertOccurredAt(input.occurredAt, deps.now(), 'occurredAt'); if (!Number.isSafeInteger(input.amountCents) || input.amountCents <= 0) throw new FinanceValidationError([{ path: 'amountCents', message: '必须是正整数分' }])
    if (input.attachment && (input.attachment.sizeBytes < 1 || input.attachment.sizeBytes > 5 * 1024 * 1024 || !['application/pdf', 'image/jpeg', 'image/png'].includes(input.attachment.mimeType) || !/(mock|demo|fake|演示|虚构)/i.test(input.attachment.name))) throw new FinanceValidationError([{ path: 'attachment', message: '仅允许单个不超过 5MB 的虚构 pdf/jpg/png 元数据' }])
    return deps.repository.transact((state) => {
      const prior = replay(state, input.requestId, 'supplier-payment-create', () => { const ids = state.requests.find((item) => item.requestId === input.requestId)!.targetIds; return { payment: state.supplierPayments.find((item) => item.id === ids[0])!, writeoff: state.supplierPaymentWriteoffs.find((item) => item.id === ids[1]) ?? null } }); if (prior) return prior
      const account = accountById(state, input.accountId); if (account.type !== input.method || account.status !== 'enabled') throw new FinanceDomainError('INVALID_STATE', '付款账户必须启用且类型与付款方式一致'); if (input.method === 'bank' && !state.banks.some((item) => item.accountId === account.id && item.status === 'enabled')) throw new FinanceDomainError('INVALID_STATE', '银行转账账户缺少启用银行资料')
      const paymentId = deps.nextId('payment'); const paymentNo = nextBusinessNo(state, 'payment', input.occurredAt); const movement = appendMovement(state, actor, { requestId: `${input.requestId}:movement`, accountId: account.id, direction: 'expense', kind: 'payment', amountCents: input.amountCents, sourceId: paymentId, sourceNo: paymentNo, counterparty: input.supplierSnapshot.name, summary: `供应商付款 ${paymentNo}`, occurredAt: input.occurredAt })
      const payment: SupplierPayment = { id: paymentId, enterpriseId: state.enterpriseId, paymentNo, supplierSnapshot: supplierSnapshot(input.supplierSnapshot), occurredAt: input.occurredAt, amountCents: input.amountCents, method: input.method, accountId: account.id, movementId: movement.id, attachment: input.attachment ? structuredClone(input.attachment) : null, note: input.note?.trim() || null, status: 'normal', voidInfo: null, requestId: input.requestId, operatorSnapshot: actorSnapshot(actor, deps), version: 1 }; state.supplierPayments.push(payment)
      let writeoff: SupplierPaymentWriteoff | null = null; if (input.immediateAllocations?.length) { const allocations = input.immediateAllocations.map((item) => ({ paymentId, payableId: item.payableId, amountCents: item.amountCents })); writeoff = buildSupplierWriteoff(state, actor, `${input.requestId}:writeoff`, input.supplierSnapshot.id, input.occurredAt, allocations) }
      state.version += 1; recordRequest(state, input.requestId, 'supplier-payment-create', [payment.id, ...(writeoff ? [writeoff.id] : [])], deps.now()); state.auditLogs.push({ id: deps.nextId('audit'), enterpriseId: state.enterpriseId, action: 'supplier-payment.created', targetId: payment.id, operatorSnapshot: actorSnapshot(actor, deps), detail: `确认供应商付款 ${payment.paymentNo}${writeoff ? ` 并核销 ${writeoff.writeoffNo}` : ''}`, createdAt: deps.now() }); return { payment, writeoff }
    })
  }

  function buildSupplierWriteoff(state: FinanceFeatureState, actor: FinanceActor, requestId: string, supplierId: string, occurredAt: string, inputs: Array<Omit<import('../types').SupplierPaymentWriteoffAllocation, 'id'>>): SupplierPaymentWriteoff {
    if (!inputs.length) throw new FinanceValidationError([{ path: 'allocations', message: '至少需要一条分配' }]); const payableTotals = new Map<string, number>(); const paymentTotals = new Map<string, number>()
    for (const item of inputs) { if (!Number.isSafeInteger(item.amountCents) || item.amountCents <= 0) throw new FinanceValidationError([{ path: 'allocations.amountCents', message: '必须是正整数分' }]); const payment = state.supplierPayments.find((entry) => entry.id === item.paymentId); const payable = state.payables.find((entry) => entry.id === item.payableId); if (!payment || payment.supplierSnapshot.id !== supplierId) throw new FinanceDomainError('INVALID_STATE', '付款不存在或供应商不一致'); if (!payable || payable.supplierSnapshot.id !== supplierId || payable.status === 'voided') throw new FinanceDomainError('INVALID_STATE', '应付不存在或供应商不一致'); if (datePart(occurredAt) < datePart(payment.occurredAt) || datePart(occurredAt) < datePart(payable.occurredAt) || occurredAt > deps.now()) throw new FinanceDomainError('INVALID_STATE', '核销日期不能早于来源或晚于当前时间'); paymentTotals.set(item.paymentId, (paymentTotals.get(item.paymentId) ?? 0) + item.amountCents); payableTotals.set(item.payableId, (payableTotals.get(item.payableId) ?? 0) + item.amountCents) }
    for (const [id, amount] of paymentTotals) if (amount > supplierPaymentAvailable(state, id)) throw new FinanceDomainError('INVALID_STATE', '核销金额超过付款可用余额'); for (const [id, amount] of payableTotals) { const payable = state.payables.find((item) => item.id === id)!; if (amount > projectSupplierPayable(state, payable, deps.now()).outstandingCents) throw new FinanceDomainError('INVALID_STATE', '核销金额超过应付待付金额') }
    const allocations = inputs.map((item) => ({ ...item, id: deps.nextId('allocation') })); const value: SupplierPaymentWriteoff = { id: deps.nextId('supplier-writeoff'), enterpriseId: state.enterpriseId, writeoffNo: nextBusinessNo(state, 'writeoff', occurredAt), supplierId, occurredAt, allocations, amountCents: allocations.reduce((sum, item) => sum + item.amountCents, 0), status: 'active', cancelInfo: null, requestId, operatorSnapshot: actorSnapshot(actor, deps), version: 1 }; state.supplierPaymentWriteoffs.push(value); return value
  }

  function createSupplierPaymentWriteoff(actor: FinanceActor, input: CreateSupplierPaymentWriteoffInput): SupplierPaymentWriteoff { permission(actor, 'finance.manage-supplier-writeoffs'); return deps.repository.transact((state) => { const prior = replay(state, input.requestId, 'supplier-writeoff-create', () => state.supplierPaymentWriteoffs.find((item) => item.id === state.requests.find((entry) => entry.requestId === input.requestId)!.targetIds[0])!); if (prior) return prior; const value = buildSupplierWriteoff(state, actor, input.requestId, input.supplierId, input.occurredAt, input.allocations); state.version += 1; recordRequest(state, input.requestId, 'supplier-writeoff-create', [value.id], deps.now()); state.auditLogs.push({ id: deps.nextId('audit'), enterpriseId: state.enterpriseId, action: 'supplier-writeoff.created', targetId: value.id, operatorSnapshot: actorSnapshot(actor, deps), detail: `新增供应商付款核销 ${value.writeoffNo}`, createdAt: deps.now() }); return value }) }
  function listSupplierPayments(actor: FinanceActor): Array<SupplierPayment & { allocatedCents: number; availableCents: number }> { permission(actor, 'finance.view-supplier-payments'); const state = deps.repository.read(); return state.supplierPayments.sort((a, b) => b.occurredAt.localeCompare(a.occurredAt)).map((item) => ({ ...structuredClone(item), allocatedCents: item.status === 'void' ? 0 : item.amountCents - supplierPaymentAvailable(state, item.id), availableCents: supplierPaymentAvailable(state, item.id) })) }
  function listSupplierPaymentWriteoffs(actor: FinanceActor): SupplierPaymentWriteoff[] { permission(actor, 'finance.view-supplier-writeoffs'); return deps.repository.read().supplierPaymentWriteoffs.sort((a, b) => b.occurredAt.localeCompare(a.occurredAt)).map((item) => structuredClone(item)) }
  function voidSupplierPayment(actor: FinanceActor, input: VoidSupplierPaymentInput): SupplierPayment {
    permission(actor, 'finance.manage-supplier-payments'); const reason = input.reason.trim(); if (!reason) throw new FinanceValidationError([{ path: 'reason', message: '作废原因不能为空' }]); const now = deps.now()
    return deps.repository.transact((state) => { const prior = replay(state, input.requestId, 'supplier-payment-void', () => state.supplierPayments.find((item) => item.id === state.requests.find((entry) => entry.requestId === input.requestId)!.targetIds[0])!); if (prior) return prior; const payment = state.supplierPayments.find((item) => item.id === input.paymentId); if (!payment) throw new FinanceDomainError('NOT_FOUND', '付款单不存在'); if (payment.version !== input.expectedVersion) throw new FinanceDomainError('CONFLICT', '付款单已变化，请刷新'); if (payment.status !== 'normal') throw new FinanceDomainError('INVALID_STATE', '付款单已经作废'); if (monthPart(payment.occurredAt) !== currentMonth() || activePeriod(state, monthPart(payment.occurredAt))) throw new FinanceDomainError('INVALID_STATE', '只能作废当前开放自然月的付款'); if (state.supplierPaymentWriteoffs.some((item) => item.status === 'active' && item.allocations.some((part) => part.paymentId === payment.id))) throw new FinanceDomainError('INVALID_STATE', '存在有效核销的付款不能作废'); appendMovement(state, actor, { requestId: `${input.requestId}:movement`, accountId: payment.accountId, direction: 'income', kind: 'payment', amountCents: payment.amountCents, sourceId: payment.id, sourceNo: payment.paymentNo, counterparty: payment.supplierSnapshot.name, summary: `作废供应商付款 ${payment.paymentNo}：${reason}`, occurredAt: now }); payment.status = 'void'; payment.voidInfo = { reason, voidedAt: now, voidedBy: actorSnapshot(actor, deps), reversalMovementId: state.movements.at(-1)?.id ?? null }; payment.version += 1; state.version += 1; recordRequest(state, input.requestId, 'supplier-payment-void', [payment.id], now); state.auditLogs.push({ id: deps.nextId('audit'), enterpriseId: state.enterpriseId, action: 'supplier-payment.voided', targetId: payment.id, operatorSnapshot: actorSnapshot(actor, deps), detail: `作废供应商付款 ${payment.paymentNo}：${reason}`, createdAt: now }); return payment })
  }
  function cancelSupplierPaymentWriteoff(actor: FinanceActor, input: CancelSupplierPaymentWriteoffInput): SupplierPaymentWriteoff {
    permission(actor, 'finance.manage-supplier-writeoffs'); const reason = input.reason.trim(); if (!reason) throw new FinanceValidationError([{ path: 'reason', message: '取消原因不能为空' }]); const now = deps.now()
    return deps.repository.transact((state) => { const prior = replay(state, input.requestId, 'supplier-writeoff-cancel', () => state.supplierPaymentWriteoffs.find((item) => item.id === state.requests.find((entry) => entry.requestId === input.requestId)!.targetIds[0])!); if (prior) return prior; const writeoff = state.supplierPaymentWriteoffs.find((item) => item.id === input.writeoffId); if (!writeoff) throw new FinanceDomainError('NOT_FOUND', '供应商核销单不存在'); if (writeoff.version !== input.expectedVersion) throw new FinanceDomainError('CONFLICT', '核销单已变化'); if (writeoff.status !== 'active') throw new FinanceDomainError('INVALID_STATE', '核销单已经取消'); if (monthPart(writeoff.occurredAt) !== currentMonth() || activePeriod(state, monthPart(writeoff.occurredAt))) throw new FinanceDomainError('INVALID_STATE', '只能取消当前开放自然月的核销'); if (writeoff.allocations.some((item) => state.supplierPayments.find((payment) => payment.id === item.paymentId)?.status === 'void')) throw new FinanceDomainError('INVALID_STATE', '付款已作废，不能取消核销'); writeoff.status = 'cancelled'; writeoff.cancelInfo = { reason, cancelledAt: now, cancelledBy: actorSnapshot(actor, deps) }; writeoff.version += 1; state.version += 1; recordRequest(state, input.requestId, 'supplier-writeoff-cancel', [writeoff.id], now); state.auditLogs.push({ id: deps.nextId('audit'), enterpriseId: state.enterpriseId, action: 'supplier-writeoff.cancelled', targetId: writeoff.id, operatorSnapshot: actorSnapshot(actor, deps), detail: `取消供应商核销 ${writeoff.writeoffNo}：${reason}`, createdAt: now }); return writeoff })
  }
  function createSupplierPayableCredit(actor: FinanceActor, input: CreateSupplierPayableCreditInput): SupplierPayableCredit {
    permission(actor, 'finance.manage-supplier-payments'); assertOccurredAt(input.occurredAt, deps.now(), 'occurredAt'); if (!Number.isSafeInteger(input.amountCents) || input.amountCents <= 0) throw new FinanceValidationError([{ path: 'amountCents', message: '必须是正整数分' }])
    return deps.repository.transact((state) => { const prior = replay(state, input.requestId, 'supplier-credit-create', () => state.supplierPayableCredits.find((item) => item.id === state.requests.find((entry) => entry.requestId === input.requestId)!.targetIds[0])!); if (prior) return prior; let outstandingReductionCents = 0; if (input.payableId) { const payable = state.payables.find((item) => item.id === input.payableId); if (!payable || payable.supplierSnapshot.id !== input.supplierId) throw new FinanceDomainError('INVALID_STATE', '贷项应付不存在或供应商不一致'); outstandingReductionCents = Math.min(input.amountCents, projectSupplierPayable(state, payable, deps.now()).outstandingCents) } const value: SupplierPayableCredit = { id: deps.nextId('supplier-credit'), enterpriseId: state.enterpriseId, sourceType: input.sourceType, sourceId: input.sourceId, sourceNo: input.sourceNo.trim(), supplierId: input.supplierId, payableId: input.payableId ?? null, amountCents: input.amountCents, outstandingReductionCents, refundObligationCents: input.amountCents - outstandingReductionCents, occurredAt: input.occurredAt, requestId: input.requestId, operatorSnapshot: actorSnapshot(actor, deps), status: 'active', reversalInfo: null, version: 1 }; state.supplierPayableCredits.push(value); state.version += 1; recordRequest(state, input.requestId, 'supplier-credit-create', [value.id], deps.now()); state.auditLogs.push({ id: deps.nextId('audit'), enterpriseId: state.enterpriseId, action: 'supplier-credit.created', targetId: value.id, operatorSnapshot: actorSnapshot(actor, deps), detail: `新增供应商应付贷项 ${value.sourceNo}`, createdAt: deps.now() }); return value })
  }
  ...
  return { listAccounts, getAccountDetail, listPeriodCards, listBanks, listPaymentChannels, saveAccount, postMovement, closePeriod, reversePeriod, saveBank, submitPaymentApplication, createOrderReceivable, listCustomerReceivables, listReceivableDocuments, listReceivableProducts, listReceivableAging, exportReceivables, listSettlementSources, listReceipts, getReceipt, listWriteoffs, getWriteoff, createReceipt, createWriteoff, voidReceipt, cancelWriteoff, createSupplierPayable, listSupplierPayables, listSupplierPayableProducts, listSupplierPayableAging, createSupplierPayment, listSupplierPayments, voidSupplierPayment, createSupplierPaymentWriteoff, listSupplierPaymentWriteoffs, cancelSupplierPaymentWriteoff, createSupplierPayableCredit, getOrderSettlement }
}
