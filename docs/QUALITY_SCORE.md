...
| 订单 | C | `ORD-001` 列表、详情、历史快照、筛选、角色遮蔽、CSV 与打印已完成；新增、审核、出库和收款未实现 | 建单、审核、出库到应收链路可重复 |
...
| Mock 与 Repository | B | 客户、商品、库存与 `ORD-001` 共用唯一 baseline；订单快照、事务打印、可控时钟、六类场景和重置哈希已有证据 |
| Service 规则 | B | 客户、商品、价格、授权、铺货/模板、库存及订单只读核心已接入；订单筛选、遮蔽、provider 失败隔离和幂等打印有自动证据 |
| 前端体验 | B | 应用壳、客户、商品、库存与订单工作区已落地；ORD-001 在 1280px 无页面溢出，宽表内部滚动、六场景、角色遮蔽和控制台通过 |
...
可以说：Harness 具备可追踪原始资料、完整功能目录、规格准备门、人工决策协议、计划恢复和自动验证；`ORD-001` 已按上述证据完成。
