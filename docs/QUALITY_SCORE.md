...
- 最近审计：2026-08-14
- 当前阶段：`ORD-003` 订单审核与状态流转正在 specification；完整 source-only 契约已提取，等待五组人工决策确认，尚未编码。
...
| 已达到 ready 的业务切片 | 8 | `CUS-001`、`PRD-001/002/003/004`、`INV-001`、`ORD-001/002` 均已 passing |
| 因需求不足 blocked | 2 | `MER-001` 商家、`ANA-001` 独立数据分析 |
| 已知人工决策门 | 109 | `DEC-PRD-001`、`DEC-ORD-002/004/014～027` 已确认；新增 `DEC-ORD-028～031` 与既有 `DEC-ORD-003` 共同阻塞 ORD-003 编码 |
| 当前业务 active plan | 1 | `ORD-003` specification plan 已创建且机械验证通过；当前等待人工决策 |

`ORD-001/002` 的已确认订单决策均有实现与验证证据；`ORD-003` 已完成 source-only 契约提取但未达到 ready，`PRD-006` 仍保持 source-only。
...
| 订单 | B | `ORD-001/002` 已完成列表、详情、历史快照、筛选、角色遮蔽、CSV、打印、新增/修改、定价核算、预收占用、模板/扫码/粘贴、并发保护与安全分享；审核、出库和收款未实现 | 完成审核、出库到应收链路后再评估 A |
...
| Mock 与 Repository | B | 客户、商品、库存与订单共用唯一 baseline；订单快照、事务打印/保存、日序列、预收占用、分享哈希、八类场景和重置哈希已有证据 |
| Service 规则 | B | 客户、商品、价格、授权、铺货/模板、库存与 ORD-001/002 已接入；金额、权限、幂等、乐观锁、provider 整体回滚和分享白名单有自动证据 |
| 前端体验 | B | 应用壳、客户、商品、库存与订单工作区已落地；ORD-002 在 1280px 与 390px 无页面溢出，表单/详情/分享黄金路径和控制台通过 |
...
可以说：Harness 具备可追踪原始资料、完整功能目录、规格准备门、人工决策协议、计划恢复和自动验证；`ORD-001/002` 已按上述证据完成。
