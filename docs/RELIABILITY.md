...
每个业务切片都必须从低到高验证，不能只验证页面能打开：

1. 契约：Types、Mock schema、枚举、必填和边界值与字段契约一致。
2. Repository：正常、空、失败、慢响应、无权限和重置场景确定可复现。
3. Service：每条关联业务规则 ID、合法/非法状态、金额与数量边界均有测试。
4. Runtime：重复提交、加载、失败重试、切换筛选和离开页面不会产生错误现场。
5. UI：字段、操作矩阵、键盘、1280px 布局和所有页面状态符合契约。
6. 黄金旅程：从重置基线开始执行完整用户任务，核对可见结果和底层数据变化。
7. 干净重启：停止服务、保留或按计划重置现场、重新启动后再次复现关键路径。

验证证据写入当前 active plan，至少记录命令、结果、测试名、Mock 场景、人工步骤、截图路径、失败修复和重跑结果。
