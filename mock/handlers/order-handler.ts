...
export function createOrderMockSession(scenarioName:OrderScenarioName='normal'){
  ...
  let seeding=true
  const financeReceivables={checkpoint:()=>financeRepository.read(),rollback:(checkpoint:unknown)=>financeRepository.reset(checkpoint as FinanceFeatureState),createFromShipment:({order,occurredAt,requestId,actor,actorName}:Parameters<NonNullable<Parameters<typeof createOrderFulfillmentService>[0]['financeReceivables']>['createFromShipment']>[0])=>{const existing=financeService.getOrderSettlement(order.id)?.receivable;if(existing)return{id:existing.id,orderId:existing.orderId,orderNo:existing.orderNo,customerSnapshot:{id:existing.customerSnapshot.id,code:existing.customerSnapshot.code,name:existing.customerSnapshot.name,categoryId:order.customerSnapshot.categoryId},amountCents:existing.amountCents,occurredAt:existing.occurredAt,requestId:existing.requestId,source:'order-shipment' as const,status:existing.status};const value=financeService.createOrderReceivable({requestId,orderId:order.id,orderNo:order.orderNo,customerSnapshot:{id:order.customerSnapshot.id,code:order.customerSnapshot.code,name:order.customerSnapshot.name,contactName:order.shippingSnapshot.recipient,phone:order.shippingSnapshot.phone},items:order.lines.map((line)=>({orderLineId:line.id,skuId:line.skuId,skuCode:line.skuCodeSnapshot,productName:line.productNameSnapshot,specification:line.specificationSnapshot,unitName:line.unitSnapshot.name,quantityMilli:line.quantityMilli,unitPriceCents:line.dealUnitPriceCents,subtotalCents:line.subtotalCents})),goodsAmountCents:order.amounts.orderAmountCents-order.amounts.freightCents,freightCents:order.amounts.freightCents,amountCents:order.amounts.orderAmountCents,settlementMethod:order.settlementSnapshot.method,paymentTermDays:order.settlementSnapshot.creditTermDays,occurredAt,operator:{id:actor.actorId,name:actorName,role:actor.role}});return{id:value.id,orderId:value.orderId,orderNo:value.orderNo,customerSnapshot:structuredClone(order.customerSnapshot),amountCents:value.amountCents,occurredAt:value.occurredAt,requestId:value.requestId,source:'order-shipment' as const,status:'open' as const}},getByOrder:(orderId:string)=>{const value=financeService.getOrderSettlement(orderId)?.receivable;if(!value)return null;const order=repository.read().orders.find((item)=>item.id===orderId);return{id:value.id,orderId:value.orderId,orderNo:value.orderNo,customerSnapshot:order?structuredClone(order.customerSnapshot):{id:value.customerSnapshot.id,code:value.customerSnapshot.code,name:value.customerSnapshot.name,categoryId:'unknown'},amountCents:value.amountCents,occurredAt:value.occurredAt,requestId:value.requestId,source:'order-shipment' as const,status:value.status}}}
