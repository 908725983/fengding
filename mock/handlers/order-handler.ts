...
import { createInventoryCatalogProvider, createInventoryMockSession } from './inventory-handler'
import { createFinanceMockSession, financeBaseline } from './finance-handler'
...
export function createOrderCustomerProvider(partialFailure=false,readState:()=>CustomerFeatureState=()=>customerBaseline):OrderCustomerProvider{
  const fail=()=>{if(partialFailure)throw new Error('原型模拟：客户服务不可用')}
  const descendants=(state:CustomerFeatureState,id:string):string[]=>state.categories.filter((item)=>item.parentId===id).flatMap((item)=>[item.id,...descendants(state,item.id)])
  const records=(state:CustomerFeatureState)=>state.customers.map((item)=>{const category=state.categories.find((entry)=>entry.id===item.categoryId);return{id:item.id,code:item.code,name:item.name,status:item.status,categoryId:item.categoryId,categoryDiscountPercent:category?.discountRatePercent??100,minimumOrderAmountCents:category?.minimumOrderAmountCents??null,salespersonId:item.salespersonId,settlementMethod:item.settlementMethod,paymentTermDays:item.paymentTermDays,creditLimitCents:item.creditLimitCents,recipient:item.primaryContactName,phone:item.primaryPhone,province:item.provinceCode,city:item.cityCode,district:item.districtCode,address:item.address,invoiceTitle:item.invoiceTitle}})
  return{categoryDescendants:(id)=>{fail();const state=readState();return descendants(state,id)},getCreditLimitCents:(id)=>{fail();return readState().customers.find((item)=>item.id===id)?.creditLimitCents??null},listOrderCustomers:()=>{fail();return structuredClone(records(readState()))},getOrderCustomer:(id)=>{fail();return structuredClone(records(readState()).find((item)=>item.id===id)??null)}}
}
...
export interface OrderMockSharedDependencies {
...
export interface OrderMockSessionOptions {
  initialState?: OrderFeatureState
  seedDemoData?: boolean
  idNamespace?: string
}

export const orderStaffValues=[{id:'admin-demo',name:'演示系统管理员'},{id:'sales-supervisor-demo',name:'演示销售主管'},{id:'salesperson-demo',name:'演示业务员'},{id:'warehouse-demo',name:'演示仓库员'},{id:'finance-demo',name:'演示财务员'},{id:'staff-demo-1',name:'演示业务员甲'},{id:'staff-demo-2',name:'演示业务员乙'}]

export function createOrderMockSession(scenarioName:OrderScenarioName='normal',seedReturnData=false,shared?:OrderMockSharedDependencies,options:OrderMockSessionOptions={}){
  const state=structuredClone(options.initialState??orderBaseline);if(scenarioName==='empty'){state.orders=[];state.printRequests=[];state.saveRequests=[];state.shares=[];state.returns=[];state.returnRequests=[];state.nextReturnSequenceByDate={}}
  const repository=new InMemoryOrderRepository(state);const nextSequence=createRuntimeSequence();const nextTokenSequence=createRuntimeSequence();const nextInventorySequence=createRuntimeSequence();const nextFinanceSequence=createRuntimeSequence();let concurrentApplied=false;const idNamespace=options.idNamespace??'runtime';const nextDomainId=(kind:string)=>`${kind}-${idNamespace}-${nextSequence()}`
  ...
  const catalog=shared?.catalog??createOrderCatalogProvider(partial)
  const prices:OrderPriceProvider={resolvePrice:(customerId,skuId,unitId,quantity)=>{if(partial)throw new Error('原型模拟：价格服务不可用');try{const value=pricing.service.resolvePrice({role:'salesperson',actorId:'order-price-provider'},{customerId,skuId,unitId,quantity});return{unitPriceCents:value.unitPriceCents,source:value.source,sourceReferenceId:value.sourceReferenceId}}catch(cause){const sku=catalog.getSku(skuId);const unit=sku?.units.find((item)=>item.id===unitId);if(sku?.marketPriceCents!==null&&sku?.marketPriceCents!==undefined&&unit)return{unitPriceCents:Math.round(sku.marketPriceCents*unit.conversionRateMilli/1000),source:'base-order',sourceReferenceId:null};throw cause}}}
  const authorizations:OrderAuthorizationProvider=shared?.authorizations??{resolveAuthorization:(customerId,productId,at)=>{if(partial)throw new Error('原型模拟：授权服务不可用');const value=authorization.service.resolveAuthorization({role:'salesperson',actorId:'order-authorization-provider'},customerId,productId,at);return{orderable:value.orderable,reason:value.reason}}}
  const warehouses:OrderWarehouseProvider={listWarehouses:()=>{if(partial)throw new Error('原型模拟：仓库服务不可用');const source=shared?.inventory.repository.read().warehouses??inventoryBaseline.warehouses;return structuredClone(source.map(({id,code,name,status,saleProhibited,type})=>({id,code,name,status,saleProhibited,type})))},getWarehouse:(id)=>{if(partial)throw new Error('原型模拟：仓库服务不可用');const source=shared?.inventory.repository.read().warehouses??inventoryBaseline.warehouses;const item=source.find((entry)=>entry.id===id);return item?structuredClone({id:item.id,code:item.code,name:item.name,status:item.status,saleProhibited:item.saleProhibited,type:item.type}):null}}
  const staff:OrderStaffProvider={listStaff:()=>structuredClone(orderStaffValues),getStaff:(id)=>structuredClone(orderStaffValues.find((item)=>item.id===id)??null)}
  ...
  const inventoryRepository=shared?.inventory.repository??new InMemoryInventoryRepository(structuredClone(inventoryBaseline))
  const inventory=shared?.inventory.service??createInventoryService({repository:inventoryRepository,catalog:createInventoryCatalogProvider(false),now:()=>orderClock,nextId:(kind)=>`${kind}-order-runtime-${nextInventorySequence()}`})
  const financeRepository=shared?.finance.repository??new InMemoryFinanceRepository(structuredClone(financeBaseline))
  const financeService=shared?.finance.service??createFinanceService({repository:financeRepository,now:()=>orderClock,nextId:(kind)=>`${kind}-order-finance-${nextFinanceSequence()}`,actorName:(actor)=>staff.getStaff(actor.actorId)?.name??actor.actorId})
  const finance:OrderFinanceProvider=shared?{getCustomerSummary:(customerId)=>{if(partial)throw new Error('原型模拟：资金服务不可用');const actor={role:'super-admin' as const,actorId:'order-finance-provider'};const receivables=financeService.listCustomerReceivables(actor).find((item)=>item.customer.id===customerId);const prepaymentBalanceCents=financeService.listSettlementSources(actor,customerId).filter((item)=>item.bucket==='available').reduce((sum,item)=>sum+item.availableCents,0);return{receivablesCents:receivables?.outstandingCents??0,prepaymentBalanceCents}}}:createDeterministicOrderFinanceProvider(partial)
  const service=createOrderService({repository,customers:shared?.customers??createOrderCustomerProvider(partial),finance,catalog,prices,authorizations,warehouses,staff,templates,now:()=>orderClock,nextId:nextDomainId,nextToken:()=>{const seed=hashOrderShareToken(`fengding-order-share-secret:${idNamespace}:${nextTokenSequence()}`);return`ord_share_${seed.slice(6)}_8f3c1d72a9b4e6f0`},hashToken:hashOrderShareToken,shareBaseUrl:'http://127.0.0.1:4173'})
  const financeRefundNamespace=shared?'app-order-refund':'order-refund'
  const financeRefunds=createFinanceRefundService({repository:financeRepository,now:()=>orderClock,nextId:(kind)=>`${kind}-${financeRefundNamespace}-${nextFinanceSequence()}`,actorName:(actor)=>staff.getStaff(actor.actorId)?.name??actor.actorId})
  let seeding=true
  ...
  const fulfillment=createOrderFulfillmentService({repository,inventoryRepository,inventory,staff,now:()=>orderClock,nextId:nextDomainId,inventoryGate:()=>seeding?'available':scenarioName==='partial-failure'?'unavailable':scenarioName==='boundary'?'locked':'available',financeGate:()=>seeding?'available':scenarioName==='partial-failure'?'unavailable':'available',financeReceivables:financeReceivablesWithRefund})
  if(options.seedDemoData!==false&&scenarioName!=='empty')seedFulfillmentBaseline(repository,fulfillment)
  const returns=createOrderReturnService({repository,now:()=>orderClock,nextId:nextDomainId,actorName:(actor)=>staff.getStaff(actor.actorId)?.name??actor.actorId,getWarehouse:(id)=>{const value=inventoryRepository.read().warehouses.find((item)=>item.id===id);return value?{id:value.id,code:value.code,name:value.name,status:value.status}:null}})
  const returnCoordinator=createOrderReturnCoordinator({orderRepository:repository,inventoryRepository,financeRepository,inventory,financeRefunds,now:()=>orderClock,nextId:nextDomainId,actorName:(actor)=>staff.getStaff(actor.actorId)?.name??actor.actorId,inventoryGate:()=>seeding?'available':scenarioName==='partial-failure'?'unavailable':'available',financeGate:()=>seeding?'available':scenarioName==='partial-failure'?'unavailable':'available'})
  if(options.seedDemoData!==false&&seedReturnData&&scenarioName!=='empty')seedReturnBaseline(repository,inventoryRepository,returns,returnCoordinator,financeRefunds)
  seeding=false
