...
export function createOrderMockSession(scenarioName:OrderScenarioName='normal',seedReturnData=false,shared?:OrderMockSharedDependencies,options:OrderMockSessionOptions={}){
  ...
  const repository=new InMemoryOrderRepository(state);let sequence=1;let tokenSequence=1;let concurrentApplied=false;const idNamespace=options.idNamespace??'runtime';const nextDomainId=(kind:string)=>`${kind}-${idNamespace}-${sequence++}`
  ...
  const inventoryRepository=shared?.inventory.repository??new InMemoryInventoryRepository(structuredClone(inventoryBaseline));let inventorySequence=1
  const inventory=shared?.inventory.service??createInventoryService({repository:inventoryRepository,catalog:createInventoryCatalogProvider(false),now:()=>orderClock,nextId:(kind)=>`${kind}-order-runtime-${inventorySequence++}`})
  const financeRepository=shared?.finance.repository??new InMemoryFinanceRepository(structuredClone(financeBaseline));let financeSequence=1
  const financeService=shared?.finance.service??createFinanceService({repository:financeRepository,now:()=>orderClock,nextId:(kind)=>`${kind}-order-finance-${financeSequence++}`,actorName:(actor)=>staff.getStaff(actor.actorId)?.name??actor.actorId})
  ...
  const service=createOrderService({repository,customers:shared?.customers??createOrderCustomerProvider(partial),finance,catalog,prices,authorizations,warehouses,staff,templates,now:()=>orderClock,nextId:nextDomainId,nextToken:()=>{const seed=hashOrderShareToken(`fengding-order-share-secret:${idNamespace}:${tokenSequence++}`);return`ord_share_${seed.slice(6)}_8f3c1d72a9b4e6f0`},hashToken:hashOrderShareToken,shareBaseUrl:'http://127.0.0.1:4173'})
  const financeRefundNamespace=shared?'app-order-refund':'order-refund'
  const financeRefunds=createFinanceRefundService({repository:financeRepository,now:()=>orderClock,nextId:(kind)=>`${kind}-${financeRefundNamespace}-${financeSequence++}`,actorName:(actor)=>staff.getStaff(actor.actorId)?.name??actor.actorId})
  let seeding=true
