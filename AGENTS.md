...
- 同一时间只推进一个业务计划；可按 `docs/PLANS.md` 合并同领域、同事实模型的低/中风险切片，但批次内每个功能都要独立通过来源、依赖、决策与恢复门，不为展示进度提前生成无契约页面。
...
只有在行为已实现、按风险等级验证、证据已写入 active plan、文档已同步，并在业务批次收尾验证后仍可复现时，任务才算完成。普通页面不逐页重复九场景、窄屏、截图、Mock 哈希和干净重启。业务切片完成只代表模块通过；跨模块或阶段闭环还必须有 integration 计划，证明同一共享 Mock Runtime 中本次新建的业务单据走完整链路。
